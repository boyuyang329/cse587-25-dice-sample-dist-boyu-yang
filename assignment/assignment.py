...
def generate_sample(die_type_counts: Tuple[int],
    ...
    def roll(draw_type: int) -> NDArray[np.integer]:
        face_probs = die_type_face_probs[draw_type]
        faces = np.arange(len(face_probs))
        current_size =rolls_per_draw
    
        return np.random.choice(faces, current_size, p = face_probs)
